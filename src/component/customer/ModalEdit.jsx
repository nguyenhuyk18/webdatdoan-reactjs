...
import {  findWardByDistrict, getDistrictByProvince, getWardApi, saveCustomerApi, updateCustomerApi } from "../../service/api.service";
import { useEffect } from "react";
import { useNavigate } from "react-router-dom";

const ModalEditCustomer = (props) => {
const navigate = useNavigate();
    const { isModalEditOpen , handleCancleModalEdit , loadingData , listWard , listProvince , listDistrict , setListWard , setDistrict , setProvince , districtHas , provinceHas , customerDetail } = props;

    // console.log(districtHas);
    ...
    useEffect(() => {
        if(customerDetail) {
            form.setFieldsValue({
                ...
                housenumber_street : customerDetail.housenumber_street,
                province_id : provinceHas.id,
                district_id : districtHas.id,
                ward_id : customerDetail.ward_id,
                status : String(customerDetail.status),
    ...
    const onFinish = async (values) => {
        ...
        const rs = await updateCustomerApi(data);
        if(rs.status == 405) {
        ...
        if(rs.status == 201) {
            openNotificationSuccess( rs.data.message ,  'Cập nhật Khách Hàng'  , api);
            loadingData() 
            form.resetFields();
            handleCancleModalEdit();
        }
        else {
            openNotificationError(rs.data.message ,  'Cập nhật Khách Hàng'  , api);
            form.resetFields();
            handleCancleModalEdit();
        }
    }
